      ...
      <section className={`modal-panel profile-modal ${closing ? "is-closing" : ""}`} onMouseDown={(event) => event.stopPropagation()}>
        <header className="profile-modal-header">
          <div>
            ...
            <h2 className="settings-title">Profilbeállítások</h2>
          </div>
        ...
        <div className="profile-modal-content">
          <aside className="profile-hero-panel">
            <div className="profile-hero-avatar">
              {activeProfile?.avatarUrl && !avatarFailed ? (
                <img src={activeProfile.avatarUrl} alt="" onError={() => setAvatarFailed(true)} />
              ) : (
                <span>{initial}</span>
              )}
            </div>
            <div className="min-w-0 text-center">
              <h3 className="truncate text-xl font-semibold text-white">{activeProfile?.name ?? "Alap profil"}</h3>
              <p className="mt-1 text-sm text-slate-300/66">Aktív munkamenet</p>
            </div>

            <div className="profile-stat-grid">
              <ProfileStat icon={<UserRound size={16} />} label="Profil" value={String(profiles.length)} />
              <ProfileStat icon={<Layers3 size={16} />} label="Shortcut" value={String(shortcuts.length)} />
              <ProfileStat icon={<Images size={16} />} label="Teendő" value={String(todos.length)} />
            </div>
          </aside>
