    ...
    <div className="notes-workspace">
      <section className="notes-editor glass">
        <header className="notes-toolbar">
          <div><div className="settings-kicker">{t("notes.quickNote")}</div><p>{t("notes.characterCount", { count: note.length })}</p></div>
        ...
        {mode === "edit" ? <textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={t("notes.placeholder")} className="notes-editor-textarea" /> : <div className="notes-preview" dangerouslySetInnerHTML={{ __html: preview }} />}
      </section>
