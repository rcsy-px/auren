    ...
    <WidgetCard title="Teendők" icon={<CheckSquare size={18} />}>
      ...
      <form onSubmit={handleSubmit} className="mt-4 border-t border-white/8 pt-4">
        <input
          ...
          placeholder="Új hozzáadása"
          className="w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-300/45"
        />
      </form>
    </WidgetCard>
  );
...
function TodoRow({ todo }: { todo: Todo }) {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition }} className="flex items-center gap-3 text-sm">
      <button type="button" className={`h-4 w-4 rounded border ${todo.completed ? "border-sky-300 bg-sky-300" : "border-slate-300/55"}`} onClick={() => toggleTodo(todo.id)} title="Kész" />
      <span className={`min-w-0 flex-1 truncate ${todo.completed ? "text-slate-300/55 line-through" : "text-slate-100/88"}`}>{todo.text}</span>
      <button type="button" className="text-slate-300/45 hover:text-white" title="Rendezés" {...attributes} {...listeners}>
        <GripVertical size={15} />
      </button>
      <button type="button" className="text-slate-300/45 hover:text-red-200" title="Törlés" onClick={() => deleteTodo(todo.id)}>
        <Trash2 size={15} />
      </button>
    </div>
  );
